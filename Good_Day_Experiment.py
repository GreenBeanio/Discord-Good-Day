...
class Good_Day_Bot(commands.Bot):
    # Init???
    def __init__(self, command_prefix, intents, self_bot, activity):
            ...
            if message.author != self.user:
                ...
                if message.content[0:1] == "!":
                    ...
                    if message.content[0:5] == "!days":
                        ...
                        if str(user) in good_days:
                            ...
                            current_streak = good_days[user]["Streaks"][
                                "Current Streak"
                            ]
                ...
                # If it's not a command check for and record good days
                    ...
                    if "good" in str(message.content).lower():
                        ...
                        await self.Check_User_Days(
                            user=str(message.author.id), day=today
                        )
            await self.process_commands(message)

    ### Commands
    def add_commands(self):
        @self.command(name="test", help="Is a test", pass_context=True)
        async def test(ctx):
            print("test")
